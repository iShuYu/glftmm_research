...
只有第 \(n\) 层满仓，并且价格相对第 \(n\) 层成本价继续逆向偏离达到 \code{new_open_lot_crit}，才会开启第 \(n+1\) 层。
当前配置 \code{new_open_lot_crit=0.01}，也就是 1\%。
...
\begin{tikzpicture}
  ...
  \node[fulllot] (b0) at (0,-2.4) {lot n\\满仓且逆向偏离\\转 close-only};
  \node[openlot] (b1) at (5.0,-2.4) {lot n+1\\开启\\open + close};

  \draw[orangearrow] (a0.south) -- node[left,font=\small] {满仓 + 价格门槛} (b0.north);
...
例如 lot 0 是多头，成本价为 100，且已经满仓；只有当 mid 跌到 99 或更低时，lot 1 才开启。
如果 lot 0 是空头，成本价为 100，且已经满仓；只有当 mid 涨到 101 或更高时，lot 1 才开启。
...
\begin{enumerate}
  ...
  \item 只有第 \(n\) 层满仓，并且价格相对第 \(n\) 层成本价逆向偏离达到 \code{new_open_lot_crit}，才能开启第 \(n+1\) 层；此时第 \(n\) 层进入 close-only。
