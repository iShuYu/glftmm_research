...
\begin{itemize}
  \item 五层 lot：每层 \code{max_position_usdt=100000}，每层 \code{stoploss=20000}，总仓位上限 500000 USDT。
  \item 两层 lot：每层 \code{max_position_usdt=250000}，每层 \code{stoploss=25000}，总仓位上限 500000 USDT。
\end{itemize}
...
\begin{itemize}
  ...
  \item 多头满仓和空头满仓都可以触发，不要求当前一定水下，也不因为 stoploss 可能触发而阻止备用层开放；
\end{itemize}

则 \code{active_lot_count} 加 1，下一层外侧备用 lot 被开放。这个开放发生在 ticker 事件上；如果某一笔 trade 刚好让当前外层 lot 达到满仓，下一层会在后续 ticker 同步时开放。

因此，多 lot 不是同时开所有层，而是只有上一层达到任意方向满仓后才启用下一层。这个机制把加仓风险拆成一层一层向外展开的仓位单元。
...
\begin{itemize}
  ...
  \item 如果最外层非空 lot 已经满仓，并且外侧存在一个刚开放但仍为空的备用 lot，则这个空备用 lot 是当前可开仓 lot。
...
状态 JSON 里会保留 \code{lots}，包含每一层 lot 的独立状态。这里的 \code{lots} 顺序是当前逻辑顺序，而不一定是最初配置顺序。状态也会保存 \code{active_lot_count}、\code{active_lot_indices}、\code{frozen_lot_indices}、\code{max_position_lots} 和 \code{stoploss_lots}，用于断点续跑。
...
\begin{itemize}
  ...
  \item \textbf{多 lot 不是简单提高单层 max position，也不是多个独立策略。} 每一层有独立成本、独立订单簿、独立止损；但开仓权限、成交数量和 index 轮换由外层 lot 栈统一调度。
