...
PARAM_KEY_ALIAS = {
    ...
    "profit_grid": "pg",
    "profit_grid_qty_distribution": "pgq",
    "min_order_qty": "moq",
...
def _is_profit_grid_qty_distribution_row(value: Any) -> bool:
    if isinstance(value, (str, dict)):
        return True
    return isinstance(value, (list, tuple)) and bool(value) and isinstance(value[0], str)


def _normalize_profit_grid_qty_distribution_value(value: Any) -> list[str | float]:
    dist = normalize_profit_grid_qty_distribution(value)
    return [dist.kind, float(dist.param)]


def _is_profit_grid_row(value: Any) -> bool:
    if not isinstance(value, (list, tuple)) or len(value) not in (3, 4, 5):
        return False
    if any(isinstance(v, (list, tuple, dict)) for v in value[:3]):
        return False
    if len(value) >= 4 and not isinstance(value[3], str):
        return False
    if len(value) == 5 and isinstance(value[4], (list, tuple, dict)):
        return False
    return True


def _normalize_profit_grid_value(value: Any) -> list[float | int | str]:
    profit_grid, qty_dist = normalize_profit_grid_spec(value)
    if profit_grid is None:
        raise ValueError("simulation.profit_grid cannot be null")
    lower_bps, upper_bps, num_grid = profit_grid
    if len(value) == 3:
        return [float(lower_bps), float(upper_bps), int(num_grid)]
    return [
        float(lower_bps),
        float(upper_bps),
        int(num_grid),
...
def _normalize_sim_param_map(cfg: dict[str, Any]) -> dict[str, list[Any]]:
    ...
    if "profit_grid" in sim_raw:
        profit_grid_raw = sim_raw["profit_grid"]
        if _is_profit_grid_row(profit_grid_raw):
            profit_grid_rows = [profit_grid_raw]
        else:
            profit_grid_rows = ensure_list_map({"profit_grid": profit_grid_raw})["profit_grid"]
        normalized_profit_grid: list[list[float | int | str]] = []
        for row in profit_grid_rows:
            if not _is_profit_grid_row(row):
                raise ValueError(
                    "simulation.profit_grid must be [lower_bps, upper_bps, num_grid], "
                    "[lower_bps, upper_bps, num_grid, qty_kind], "
                    "[lower_bps, upper_bps, num_grid, qty_kind, qty_param], "
                    "or a list of those rows"
                )
            normalized_profit_grid.append(_normalize_profit_grid_value(row))
        sim_map["profit_grid"] = normalized_profit_grid

    if "profit_grid_qty_distribution" in sim_raw:
        qty_dist_raw = sim_raw["profit_grid_qty_distribution"]
        if _is_profit_grid_qty_distribution_row(qty_dist_raw):
            qty_dist_rows = [qty_dist_raw]
        else:
            qty_dist_rows = ensure_list_map(
                {"profit_grid_qty_distribution": qty_dist_raw}
            )["profit_grid_qty_distribution"]
        sim_map["profit_grid_qty_distribution"] = [
            _normalize_profit_grid_qty_distribution_value(row)
    ...
    if "open_curve_underwater" in sim_map:
        ...
        for row in open_curve_rows:
            if not isinstance(row, (list, tuple)) or len(row) != 3:
                raise ValueError(
                    "simulation.open_curve_underwater must be list of (min, max, order) triples"
                )
            normalized_rows.append([float(row[0]), float(row[1]), float(row[2])])
        sim_map["open_curve_underwater"] = normalized_rows
...
def _build_simulation_config(raw: dict[str, Any]) -> SimulationConfig:
    ...
    open_curve_raw = raw.get("open_curve_underwater")
    ...
    if open_curve_raw is None:
        ...
        if not isinstance(open_curve_raw, (list, tuple)) or len(open_curve_raw) != 3:
            raise ValueError("simulation.open_curve_underwater must be [min, max, order]")
        open_curve = tuple(float(v) for v in open_curve_raw)

    profit_grid_raw = raw.get("profit_grid")
    profit_grid: tuple[float, float, int] | None
    profit_grid_qty_distribution_raw = raw.get("profit_grid_qty_distribution")
    profit_grid, profit_grid_qty_distribution = normalize_profit_grid_spec(
        profit_grid_raw,
        profit_grid_qty_distribution_raw,
    )
